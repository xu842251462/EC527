/****************************************************************************
   ...
   gcc -O1 -fopenmp -std=gnu11 part2SOR.c -lpthread -lrt -lm -o part2SOR
   OMP_NUM_THREADS=4 ./part2SOR
...
void SOR_blocked_omp(arr_ptr v, int *iterations);
...
int main(int argc, char *argv[])
{
  ...
  OPTION++;
  printf("OPTION=%d (SOR_blocked_omp)\n", OPTION);
  for (x=0; x<NUM_TESTS && (n = A*x*x + B*x + C, n<=alloc_size); x++) {
    printf("  iter %d rowlen = %d\n", x, GHOST+n);
    init_array_rand(v0, GHOST+n);
    set_arr_rowlen(v0, GHOST+n);
    clock_gettime(CLOCK_REALTIME, &time_start);
    SOR_blocked_omp(v0, iterations);
    clock_gettime(CLOCK_REALTIME, &time_stop);
    time_stamp[OPTION][x] = interval(time_start, time_stop);
    convergence[OPTION][x] = *iterations;
  }
  ...
  printf("\n");
  printf("size, SOR time, SOR iters, red/black time, red/black iters, SOR_ji time, SOR_ji iters, SOR_blocked time, SOR_blocked iters, SOR_blocked_omp time, SOR_blocked_omp iters\n");
  {
    int i, j;
...
"Make sure all coefficients A, B, and C are multiples of %d\n", BLOCK_SIZE);
  ...
  while ((total_change/(double)(rowlen*rowlen)) > (double)TOL) {
    iters++;
    total_change = 0;
    #pragma omp parallel shared(data,rowlen,total_change) private(ii,jj,i,j,change)
    {
    #pragma omp for
    for (ii = 1; ii < rowlen-1; ii+=BLOCK_SIZE) {
      for (jj = 1; jj < rowlen-1; jj+=BLOCK_SIZE) {
