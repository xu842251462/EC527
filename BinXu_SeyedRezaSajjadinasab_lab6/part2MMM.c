/***********************************************************************
 ...
 gcc -O1 -fopenmp part2MMM.c -lrt -o part2MMM
 OMP_NUM_THREADS=4 ./part2MMM
...
void mmm_kij_blocked_omp(matrix_ptr a, matrix_ptr b, matrix_ptr c);
void mmm_kij_omp_v2(matrix_ptr a, matrix_ptr b, matrix_ptr c);
...
int main(int argc, char *argv[])
{
  ...
  for (OPTION = 0; OPTION<OPTIONS; OPTION++) {
    ...
    for (x=0; x<NUM_TESTS && (n = A*x*x + B*x + C, n<=alloc_size); x++) {
      ...
      switch(OPTION) {
       ...
       case 4:        mmm_kij_omp_v2(a0, b0, c0);     break;
       case 5:   mmm_kij_blocked_omp(a0, b0, c0);     break;
       default: break;
      }
  ...
  printf("rowlen, ijk, ijk_omp, kij, kij_omp, kij_omp_v2, kij_blocked_omp\n");
  {
    int i, j;
    for (i = 0; i < x; i++) {
             ...
             for (j = 0; j < row_length; j += BLOCK_SIZE){
              ...
              for (kk = k; kk < k + BLOCK_SIZE; kk++) {
                for (ii = i; ii < i + BLOCK_SIZE; ii++) {
                  r = a0[ii*row_length + kk];
                    for (jj = j; jj < j + BLOCK_SIZE; jj++) {
                            c0[ii*row_length + jj] +=  r* b0[kk*row_length + jj];
                        }
                    }
              }
            }
...
#pragma omp for
    for (k = 0; k < row_length; k++) {
      #pragma omp parallel shared(a0,b0,c0,row_length,k) private(i,j,r)
      {
      #pragma omp for
      for (i = 0; i < row_length; i++) {
       // #pragma omp parallel shared(a0,b0,c0,row_length,k,i) private(j,r)
         {
        r = a0[i*row_length+k];
       // #pragma omp for
        for (j = 0; j < row_length; j++)
