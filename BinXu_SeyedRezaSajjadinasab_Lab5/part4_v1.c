...
int main(int argc, char *argv[])
{
    ...
    for (x = 0; x < NUM_TESTS && (n = A*x*x + B*x + C, n<=alloc_size); x++)  {
        ...
        for (int i = 0; i < NUM_thread; i++) {
            data[i].vec = v0;
            data[i].arg = i;
            pthread_create(&Threads, NULL, SOR, (void*)&data[i]);
        }
        //
        for (int i = 0; i < NUM_thread; i++) {
    ...
    for (x=0; x<NUM_TESTS && (n = A*x*x + B*x + C, n<=alloc_size); x++) {
        ...
        for (int i = 0; i < NUM_thread; i++) {
            data[i].vec = v0;
            data[i].arg = i;
            pthread_create(&Threads, NULL, SOR_redblack, (void*)&data[i]);
        }
        for (int i = 0; i < NUM_thread; i++) {
            //access the thread, there is 4 thread, use pthread to access and wait every thread
            pthread_join(Threads[i], &status);
//            printf("%d/n", status);
        }
        clock_gettime(CLOCK_REALTIME, &time_stop);
    ...
    printf("\n");
    printf("size, SOR time, SOR iters, red/black time, red/black iters, SOR_ji time, SOR_ji iters, SOR_blocked time, SOR_blocked iters, SOR_strip time, SOR_strip iters\n");
    {
        int i, j;
        for (i = 0; i < NUM_TESTS; i++) {
            printf("%4d", A*i*i + B*i + C); //row length
...
/* SOR red/black with 4 thread*/
void SOR_redblack(void * arg)
{
    int i, j, redblack, iterations;
    long int ti;
    struct thread_data *data_thread = (struct thread_data*)arg;
    arr_ptr v = data_thread->vec;
    long int rowlen = get_arr_rowlen(v);
