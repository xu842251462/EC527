...
int main(int argc, char *argv[])
{
  ...
  long int x, n;
  double wd;
struct timespec time_start_psum1, time_stop_psum1;
struct timespec time_start_psum2, time_stop_psum2;
double *meas_psum1, *meas_psum2;
meas_psum1 = (double*)malloc(MAX_SIZE* sizeof(*meas_psum1));
meas_psum2 = (double*)malloc(MAX_SIZE* sizeof(*meas_psum2));
  // initialize
  ...
  for (x=0; x<NUM_TESTS && (n = A*x*x + B*x + C, n<MAX_SIZE); x++) {
    ...
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time_start_psum1);
    psum1(in, out, n);
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time_stop_psum1);
    meas_psum1[x] = interval(time_start_psum1, time_stop_psum1)*3e9;
  }
  ...
  for (x=0; x<NUM_TESTS && (n = A*x*x + B*x + C, n<MAX_SIZE); x++) {
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time_start_psum2);
    psum2(in, out, n);
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time_stop_psum2);
    meas_psum2[x] = interval(time_start_psum2, time_stop_psum2)*3e9;
  }
