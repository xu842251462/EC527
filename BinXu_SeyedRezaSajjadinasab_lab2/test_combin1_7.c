...
int main(int argc, char *argv[])
{
    ...
    for (x=0; x<NUM_TESTS && (n = A*x*x + B*x + C, n<=alloc_size); x++) {
        set_array_length(v0, n);
        clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time_start);
        for(k=0; k<OUTER_LOOPS; k++) { //num of elements
            combine2(v0, result);
            final_answer += *result;
        }
        clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time_stop);
    ...
    /* output times */
    printf("%s", "size, combine1, combine2, combine3, combine4, combine5, combine6, combine7, combine8, combine9, combine10\n");
    {
        int i, j;
        for (i = 0; i < NUM_TESTS; i++) {
            ...
            for (j = 0; j < OPTIONS; j++) {
                ...
                printf("%ld", (long int)((double)(CPNS) * 1.0e9 * time_stamp[j][i])); //num of cycles
            }
            printf("\n");
        }
    }
...
//change to unrolling 10, to test 1b
...
//testing for different operation and unroll number
...
void combine9(array_ptr v, data_t *dest)
{
    ...
    for (i = 0; i < limit; i+=5) {
        acc0 = acc0 OP (data[i] OP data[i + 1]);
        acc1 = acc1 OP (data[i + 2] OP (data[i + 3] OP data[i + 4]));
    }
...
void combine10(array_ptr v, data_t *dest)
{
    ...
    for (i = 0; i < limit; i+=6) {
        acc0 = acc0 OP (data[i] OP data[i + 1]);
        acc1 = acc0 OP (data[i + 1] OP data[i + 2]);
        acc2 = acc0 OP (data[i + 3] OP data[i + 4]);
    }
    ...
    acc = acc OP acc0 OP acc1 OP acc2;
    *dest = acc;
}