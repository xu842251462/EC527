...
void add(data_t* pA1, data_t* pA2, data_t* pR, long int nSize);
void multi(data_t* pA1, data_t* pA2, data_t* pR, long int nSize);
void add_vectorize(data_t* pA1, data_t* pA2, data_t* pR, long int nSize);
void multi_vectorize(data_t* pA1, data_t* pA2, data_t* pR, long int nSize);
void dot_intrinsics(data_t* pArray1, data_t* pArray2, data_t* pResult, long int nSize);
...
int main(int argc, char *argv[])
{
    ...
    for (x=0; x<NUM_TESTS && (n = A*x*x + B*x + C, n<=alloc_size); x++) {
        clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time_start);
        for (long k=0; k<OUTER_LOOPS; k++) {
            multi(pArray1, pArray2, pResult, n);
        }
        clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time_stop);
        time_stamp[OPTION][x] = interval(time_start, time_stop);
    }
    ...
    for (x=0; x<NUM_TESTS && (n = A*x*x + B*x + C, n<=alloc_size); x++) {
        clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time_start);
        for (long k=0; k<OUTER_LOOPS; k++) {
            add_vectorize(pArray1, pArray2, pResult, n);
        }
        clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time_stop);
        time_stamp[OPTION][x] = interval(time_start, time_stop);
    }
    ...
    for (x=0; x<NUM_TESTS && (n = A*x*x + B*x + C, n<=alloc_size); x++) {
        clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time_start);
        for (long k=0; k<OUTER_LOOPS; k++) {
            multi_vectorize(pArray1, pArray2, pResult, n);
        }
        clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time_stop);
        time_stamp[OPTION][x] = interval(time_start, time_stop);
    }
    ...
    for (x=0; x<NUM_TESTS && (n = A*x*x + B*x + C, n<=alloc_size); x++) {
        clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time_start);
        for (long k=0; k<OUTER_LOOPS; k++) {
            dot_intrinsics(pArray1, pArray2, pResult, n);
        }
        clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time_stop);
        time_stamp[OPTION][x] = interval(time_start, time_stop);
    }
    /* output times */
    printf("size, Scalar, vecSSE, vecAVX, add, multi, add_vectorize, multi_vectorize, dot_vectorize\n");
    {
        for (int i = 0; i < x; i++) {
...
void multi (data_t* pArray1,       // [in] 1st source array
            data_t* pArray2,       // [in] 2nd source array
            data_t* pResult,       // [out] result array
            long int nSize)     {
    data_t* pSource1 = pArray1;
...
void multi_vectorize(data_t* pArray1,       // [in] 1st source array
                     data_t* pArray2,       // [in] 2nd source array
                     data_t* pResult,       // [out] result array
                     long int nSize) {
    long  nLoop = nSize / 4;
...
void dot_intrinsics(data_t* pArray1,       // [in] 1st source array
                    data_t* pArray2,       // [in] 2nd source array
                    data_t* pResult,       // [out] result array
                    long int nSize) {
    long  nLoop = nSize / 4;
